...
# Function to check Hasura environment
check_hasura_env() {
    if [ ! -f "$ENGINE_DIR/.env" ]; then
        print_error "No .env file found in $ENGINE_DIR. Please create it first."
        exit 1
    fi
}

# Function to start Hasura
start_hasura() {
    ...
    wait_for_hasura "$HASURA_PID"
}

# Function to wait for Hasura to be ready
wait_for_hasura() {
    local hasura_pid=$1
